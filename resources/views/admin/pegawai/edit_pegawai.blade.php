...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            url: '{{ url("api/v1/data-pegawai-edit") }}/' + id_pegawai,
            headers: {
            ...
            success: function(res) {
                ...
                if(res.status) {
                    ...
                    let pegawai_id = res.data.id;
                    let full_name = res.data.full_name;
                    let name = res.data.name;
                    let username = res.data.username;
                    let email = res.data.email;
                    let unit_id = res.data.unit_id;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.status) {
                        ...
                        let option = '<option value="" disable>Pilih Unit</option>';
                        $.each(response.data, function(index, val) {

                            if(id_unit == val.id) {
                                option += '<option value="'+ val.id +'" title="'+ val.unit +'" selected>'+ val.unit +'</option>';
                            } else {
